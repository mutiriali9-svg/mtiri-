...
import { Bell, BellRing, X, BellDot, FileWarning, ClipboardList, CreditCard, Receipt, StickyNote, ChevronLeft, ChevronDown, Home, Building2 } from 'lucide-react';
...
export default function NotificationDropdown({
  ...
  /* ── Expired sub-items (inline for mobile, side panel for desktop) ── */
  const expiredSubItems = (
    <>
      <Link
        to="/units"
  ...
  const renderRow = (item) => {
    ...
    const rowContent = (
      <>
        ...
        {item.expandable && (
          isMobile
            ? <ChevronDown size={14} className="text-muted-foreground flex-shrink-0" style={{ transform: expiredOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
            : <ChevronLeft size={14} className="text-muted-foreground flex-shrink-0" style={{ transform: expiredOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
        )}
      </>
    );
    ...
    if (item.expandable) {
      return (
        <div key={item.key}>
          ...
          {/* موبايل: القرية والعقارات تفتح تحت مباشرة (inline) */}
          {isMobile && expiredOpen && (
            <div className="border-t border-border/50" style={{ backgroundColor: 'rgba(230,57,70,0.04)' }} dir={isRtl ? 'rtl' : 'ltr'}>
              {expiredSubItems}
            </div>
          )}
        </div>
  ...
  const mainPanel = (
    <div
      ...
      style={{ minWidth: isMobile ? 'unset' : '270px', width: isMobile ? '100%' : 'auto', order: 2 }}
      dir={isRtl ? 'rtl' : 'ltr'}
    >
  ...
  const sidePanel = !isMobile && expiredOpen && (
    <div
      className="bg-white rounded-2xl shadow-2xl border border-border overflow-hidden animate-fade-in-up"
