...
export default function Reports() {
  ...
  const monthlyData = MONTHS.map((name, idx) => {
    const gross = yearPayments
      .filter(p => getMonth(parseISO(p.payment_date)) === idx)
      .reduce((s, p) => s + (p.amount || 0), 0);
    const ref = yearRefunds
      .filter(r => getMonth(parseISO(r.applies_to_date || r.refund_date)) === idx)
      .reduce((s, r) => s + (Number(r.amount) || 0), 0);
    const rec = yearRecoveries
      .filter(x => getMonth(parseISO(x.deduction_date)) === idx)
