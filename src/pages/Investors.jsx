  ...
  return (
    <div className="space-y-6 animate-fade-in-up ">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#1B2B4B' }}>{t('investorsTitle')}</h1>
          <p className="text-xs text-muted-foreground mt-0.5">Investors & Profit Distribution</p>
        </div>
      </div>

      {/* Date Filter Bar */}
      <div className="bg-white rounded-xl card-bevel p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {[thisYear, thisYear - 1, thisYear - 2, thisYear - 3].map(y => (
            <button
              key={y}
              onClick={() => selectedYear === String(y) ? applyAllYears() : applyYearFilter(y)}
              className="px-3 py-1.5 rounded-lg text-sm font-medium border transition-all"
              style={selectedYear === String(y)
          ...
          {selectedYear && selectedYear !== 'all' && (
            <button
              onClick={applyAllYears}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm text-muted-foreground hover:bg-muted transition-colors"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <div className="flex flex-col gap-1">
            ...
            <input type="date" value={dateFrom} onChange={e => { setDateFrom(e.target.value); setSelectedYear(''); }}
              className="border border-input rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-1 focus:ring-ring w-full" />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs text-muted-foreground">{t('toDate')}</label>
            <input type="date" value={dateTo} onChange={e => { setDateTo(e.target.value); setSelectedYear(''); }}
              className="border border-input rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-1 focus:ring-ring w-full" />
          </div>
          {(dateFrom || dateTo) && !selectedYear && (
            <div className="flex flex-col gap-1 justify-end">
              <button onClick={resetFilter}
                className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg border border-input text-sm text-muted-foreground hover:bg-muted transition-colors">
      ...
      <div className="bg-white rounded-xl card-bevel overflow-hidden hidden md:block">
        <div className="px-5 py-4 border-b border-border flex items-center gap-2 flex-wrap">
          ...
          <span className="text-xs text-muted-foreground ml-auto">{t('totalShares')}: {totalShares.toFixed(2)}%</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-[11px] lg:text-sm">
            ...
            <tbody>
              {investors.sort((a, b) => b.share_percentage - a.share_percentage).map((inv, i) => {
                ...
                const revShare = totalRevenue * pct;
                const expShare = totalExpenses * pct;
                const netShare = netProfit * pct;
                const savShare = totalSavings * pct;
                return (
                  <tr key={inv.id} className={`border-t border-border/50 ${i % 2 === 0 ? '' : 'bg-muted/20'}`}>
                    ...
                    <td className="px-2.5 lg:px-5 py-3.5 whitespace-nowrap">
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold"
                        style={{ backgroundColor: '#FFF8E7', color: '#C9A84C' }}>
                        {inv.share_percentage}%
                      </span>
                    </td>
                    <td className="px-2.5 lg:px-5 py-3.5 whitespace-nowrap font-medium text-green-700">{fmt(revShare)} AED</td>
                    <td className="px-2.5 lg:px-5 py-3.5 whitespace-nowrap text-red-600">{fmt(expShare)} AED</td>
                    <td className="px-2.5 lg:px-5 py-3.5 whitespace-nowrap font-semibold" style={{ color: '#059669' }}>{fmt(savShare)} AED</td>
                    <td className="px-2.5 lg:px-5 py-3.5 whitespace-nowrap font-bold" style={{ color: netShare >= 0 ? '#2A9D8F' : '#E63946' }}>
                      {fmt(netShare)} AED
                    </td>
                  </tr>
                );
            ...
            <tfoot>
              <tr className="border-t-2 border-border bg-muted/30 font-bold text-sm">
                <td className="px-2.5 lg:px-5 py-3 whitespace-nowrap" style={{ color: '#1B2B4B' }}>{t('totalRow_i')}</td>
                <td className="px-2.5 lg:px-5 py-3 whitespace-nowrap" style={{ color: '#C9A84C' }}>{totalShares.toFixed(2)}%</td>
                <td className="px-5 py-3 text-green-700">{fmt(totalRevenue * totalShares / 100)} AED</td>
                <td className="px-5 py-3 text-red-600">{fmt(totalExpenses * totalShares / 100)} AED</td>
      ...
      <div className="md:hidden space-y-3">
        <div className="bg-white rounded-xl card-bevel overflow-hidden px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Users size={18} style={{ color: '#C9A84C' }} />
            <h2 className="font-semibold" style={{ color: '#1B2B4B' }}>{t('investorShares')}</h2>
          </div>
          <span className="text-xs text-muted-foreground">{totalShares.toFixed(2)}%</span>
        </div>
          ...
          return (
            <div key={inv.id} className="bg-white rounded-xl card-bevel p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-bold text-base" style={{ color: '#1B2B4B' }}>{inv.name}</h3>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">{t('revShare')}</span>
                  <span className="font-medium text-green-700">{fmt(revShare)} AED</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">{t('expShare')}</span>
                  <span className="font-medium text-red-600">{fmt(expShare)} AED</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">{lang === 'ar' ? 'نصيب الادخار' : 'Savings Share'}</span>
                  <span className="font-semibold" style={{ color: '#059669' }}>{fmt(savShare)} AED</span>
                </div>
                <div className="flex items-center justify-between text-base pt-2 border-t border-border">
                  ...
                  <span className="font-bold" style={{ color: netShare >= 0 ? '#2A9D8F' : '#E63946' }}>
                    {fmt(netShare)} AED
                  </span>
                </div>
              </div>
        ...
        <div className="bg-muted/30 rounded-xl card-bevel p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="font-bold text-sm" style={{ color: '#C9A84C' }}>{totalShares.toFixed(2)}%</span>
          </div>
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">{t('revShare')}</span>
              <span className="font-bold text-green-700">{fmt(totalRevenue * totalShares / 100)} AED</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">{t('expShare')}</span>
              <span className="font-bold text-red-600">{fmt(totalExpenses * totalShares / 100)} AED</span>
            </div>
            <div className="flex items-center justify-between text-base pt-2 border-t border-border">
              <span className="text-muted-foreground">{t('netShare')}</span>
              <span className="font-bold" style={{ color: '#2A9D8F' }}>{fmt(netProfit * totalShares / 100)} AED</span>
            </div>
          </div>
        </div>
      </div>
