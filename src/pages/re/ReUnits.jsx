...
export default function ReUnits() {
  ...
  const fetchUnits = useCallback(async () => {
    setLoading(true);
    const data = await base44.entities.ReUnit.list();
    setUnits(data);
    setLoading(false);
  }, []);
  ...
  const handleSave = async () => {
    ...
    if (editUnit) {
      ...
      setDialogOpen(false);
      setSaving(false);
      await base44.entities.ReUnit.update(editUnit.id, data);
      ...
      setDialogOpen(false);
      setSaving(false);
      const created = await base44.entities.ReUnit.create(data);
  ...
  const handleDelete = (id) => {
    ...
    setConfirmDelete({ message: 'هل تريد حذف هذه الوحدة؟', onConfirm: async () => {
      ...
      await base44.entities.ReUnit.delete(id);
  ...
  return (
    <div className="space-y-5 animate-fade-in-up">
      ...
      <PageHeader
        titleAr="الوحدات السكنية - العقارات"
              ...
              {loading ? Array(5).fill(0).map((_, i) => (
                ...
                return (
                  <tr key={u.id} onClick={() => setViewUnit(u)}
                    ...
                    <td className="py-3 px-4 font-medium" style={{ color: '#1B2B4B' }}>
                      {u.tenant_name || '-'}
                    ...
                    <td className="py-3 px-4 font-semibold" style={{ color: '#1B2B4B' }}>{u.annual_rent ? `${u.annual_rent.toLocaleString()} AED` : '-'}</td>
                    <td className="py-3 px-4 text-muted-foreground text-xs"><span className="truncate block max-w-32">{u.payment_plan || '-'}</span></td>
                    ...
                    <td className="py-3 px-4" onClick={ev => ev.stopPropagation()}>
                      {isAdmin && (
                        <div className="flex items-center gap-1">
                          <button onClick={() => openEdit(u)} className="p-1.5 rounded hover:bg-muted transition-colors text-muted-foreground hover:text-navy"><Edit2 size={14} /></button>
                          <button onClick={() => handleDelete(u.id)} className="p-1.5 rounded hover:bg-destructive/10 transition-colors text-muted-foreground hover:text-destructive"><Trash2 size={14} /></button>
                        </div>
                      )}
                    </td>
                  </tr>
            ...
            <div key={u.id} onClick={() => setViewUnit(u)}
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold" style={{ color: '#1B2B4B' }}>{u.annual_rent ? `${u.annual_rent.toLocaleString()} AED` : '-'}</span>
                </div>
                <div className="flex items-center justify-between">
              ...
              {isAdmin && (
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-border" onClick={ev => ev.stopPropagation()}>
                  <button onClick={() => openEdit(u)} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg hover:bg-muted transition-colors text-sm"><Edit2 size={14} />{t('edit')}</button>
                  <button onClick={() => handleDelete(u.id)} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg hover:bg-destructive/10 transition-colors text-sm text-destructive"><Trash2 size={14} />{t('delete')}</button>
                </div>
              )}
            </div>
          );
          ...
          {viewUnit && (() => {
            ...
            return (
              <div className="space-y-3 py-1">
                ...
                {isAdmin && (
                  <Button className="w-full" onClick={() => { setViewUnit(null); openEdit(viewUnit); }} style={{ backgroundColor: '#1B2B4B' }}>
                    <Edit2 size={14} /> تعديل
                  </Button>
                )}
              </div>
