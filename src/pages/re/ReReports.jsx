  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              {(availableYears.length > 0 ? availableYears : [new Date().getFullYear()]).map(y => (
                <button key={y} onClick={() => setYearFilter(y)} className="px-3 py-1.5 text-sm rounded-lg font-medium transition-all"
                  style={{ backgroundColor: yearFilter === y ? '#1B2B4B' : '#F1F5F9', color: yearFilter === y ? '#FFFFFF' : '#64748B' }}>
                  {y}
                </button>
              ))}
            </div>
          ...
          {monthlyData.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={monthlyData}>
                ...
                <YAxis tick={{ fontSize: 11, fill: '#94A3B8' }} axisLine={false} tickLine={false} tickFormatter={v => `${(v / 1000).toFixed(0)}k`} />
                <Tooltip contentStyle={{ fontFamily: 'Cairo', fontSize: 13, border: '1px solid #E2E8F0', borderRadius: 8 }} formatter={(v) => [`${v.toLocaleString()} AED`, t('net')]} />
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={expCatData} cx="50%" cy="50%" outerRadius={80} dataKey="value"
                  label={({ name, percent }) => `${catLabels[name] || name} ${(percent * 100).toFixed(0)}%`} labelLine={false} style={{ fontSize: 11, fontFamily: 'Cairo' }}>
                  {expCatData.map((_, idx) => <Cell key={idx} fill={PIE_COLORS[idx % PIE_COLORS.length]} />)}
                </Pie>
                <Tooltip contentStyle={{ fontFamily: 'Cairo', fontSize: 12, border: '1px solid #E2E8F0', borderRadius: 8 }}
      ...
      <div className="bg-white card-bevel rounded-xl p-5 hidden md:block">
        <div className="mb-4">
          <h3 className="font-bold" style={{ color: '#1B2B4B' }}>{t('monthlyFinancialSummary')} — {yearFilter}</h3>
        </div>
