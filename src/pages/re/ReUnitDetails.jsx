...
export default function ReUnitDetails() {
  ...
  Promise.all([
    base44.entities.ReUnit.filter({ unit_number: decodedUnit }),
    base44.entities.RePayment.list('-payment_date', 500),
  ]).then(([units, allPays]) => {
  ...
  const handleSave = async () => {
    ...
    await base44.entities.ReUnit.update(unit.id, data);
  ...
  const handleSavePayment = async () => {
    ...
    await base44.entities.RePayment.update(editingPayment.id, data);
