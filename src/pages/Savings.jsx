...
export default function Savings() {
  ...
  const [formData, setFormData] = useState(EXPENSE_INIT);
  const [confirmDelete, setConfirmDelete] = useState(null);
  const { t, lang } = useLang();

  const fetchData = () => {
    base44.entities.Expense.filter({ category: 'savings' }).then(data => {
      const sorted = data.sort((a, b) => new Date(b.expense_date) - new Date(a.expense_date));
      setSavingExpenses(sorted);
      setLoading(false);
    });
  };
  ...
  const handleSave = async () => {
    ...
    await base44.entities.Expense.create({ ...formData, amount: Number(formData.amount), category: 'savings' });
    toast.success(t('savingAdded'));
    setFormData(EXPENSE_INIT);
    setDialogOpen(false);
    fetchData();
  };
  ...
  const handleDelete = (id) => {
    setConfirmDelete({ message: t('confirmDeleteSaving'), onConfirm: async () => {
      ...
      setSavingExpenses(prev => prev.filter(s => s.id !== id));
      setConfirmDelete(null);
      toast.success(t('savingDeleted'));
    }});
  };

  const totalSavings = savingExpenses.reduce((sum, s) => sum + (s.amount || 0), 0);
    ...
    <div className="space-y-6 animate-fade-in-up ">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold" style={{ color: '#1B2B4B' }}>{t('savingsTitle')}</h1>
            <p className="text-xs text-muted-foreground">{t('savingsSubTitle')}</p>
          </div>
        </div>
        <Button onClick={() => { setFormData(EXPENSE_INIT); setDialogOpen(true); }} className="gap-2" style={{ backgroundColor: '#1B2B4B' }}>
      ...
      <div className="bg-white rounded-xl card-bevel p-6 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-3xl font-bold" style={{ color: '#2A9D8F' }}>{fmt(totalSavings)} AED</p>
        </div>
      </div>

      {/* Savings Table - Desktop */}
      <div className="bg-white rounded-xl card-bevel overflow-hidden hidden md:block">
        ...
        {savingExpenses.length === 0 ? (
          <div className="p-8 text-center text-muted-foreground">{t('noSavings')}</div>
        ) : (
            ...
            <table className="w-full text-sm">
                ...
                {savingExpenses.map((s, i) => (
                  <tr key={s.id} className={`border-t border-border/50 ${i % 2 === 0 ? '' : 'bg-muted/20'}`}>
                    ...
                    <td className="px-5 py-3.5">
                      <button onClick={() => handleDelete(s.id)} className="p-1.5 hover:bg-muted rounded transition-colors">
              ...
              <tfoot>
                <tr className="border-t-2 border-border bg-muted/30 font-bold">
                  ...
                  <td className="px-5 py-3" style={{ color: '#2A9D8F' }}>{fmt(totalSavings)} AED</td>
                  <td colSpan={3} />
                </tr>
              </tfoot>
            </table>
      ...
      <div className="md:hidden space-y-3">
        {savingExpenses.length === 0 ? (
          <div className="bg-white rounded-xl card-bevel p-8 text-center text-muted-foreground">{t('noSavings')}</div>
        ) : (
          savingExpenses.map((s, i) => (
            <div key={s.id} className="bg-white rounded-xl card-bevel p-4">
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <h3 className="font-bold text-base" style={{ color: '#1B2B4B' }}>{s.description}</h3>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {new Date(s.expense_date).toLocaleDateString(lang === 'ar' ? 'ar-AE' : 'en-US')}
                  </p>
                </div>
              ...
              {s.notes && (
                <p className="text-xs text-muted-foreground mb-3">{s.notes}</p>
              )}
              <button onClick={() => handleDelete(s.id)} 
                className="w-full flex items-center justify-center gap-1.5 py-2 rounded-lg hover:bg-destructive/10 transition-colors text-sm text-destructive">
        ...
        <div className="bg-muted/30 rounded-xl card-bevel p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="font-bold text-lg" style={{ color: '#2A9D8F' }}>{fmt(totalSavings)} AED</span>
          </div>
        </div>
      </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('addSavingTitle')}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <label className="text-sm font-medium text-muted-foreground">{t('savingsDescription')}</label>
              <Input
            ...
            <div>
              <label className="text-sm font-medium text-muted-foreground">{t('savingsAmount')}</label>
              <Input
                type="number"
                placeholder="0"
            ...
            <div>
              <label className="text-sm font-medium text-muted-foreground">{t('savingsDate')}</label>
              <Input
                type="date"
